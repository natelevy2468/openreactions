...
export default function EditorPopover({ label, side = 'bottom', colors, children, compact = false }) {
  ...
  useEffect(() => {
    ...
    const place = () => {
      ...
      const width = Math.min(side === 'right' ? 340 : 360, window.innerWidth - 24);
      setPosition({ width, left: Math.max(12, Math.min(side === 'right' ? box.right + 12 : box.left, window.innerWidth - width - 12)), top: side === 'right' ? Math.max(60, Math.min(box.top, window.innerHeight - 470)) : box.bottom + 8, maxHeight: side === 'right' ? Math.min(450, window.innerHeight - 80) : window.innerHeight - box.bottom - 20 });
    };
    place();
    panel.current?.focus();
  ...
  return <>
    <button ref={trigger} className={"editor-menu-trigger" + (compact ? " compact-menu-trigger" : "")} aria-label={label} aria-expanded={open} aria-controls={open ? id : undefined} aria-haspopup="dialog" onClick={() => setOpen(v => !v)} style={{ color: compact ? colors.buttonActive : colors.text, background: open ? colors.button : 'transparent' }}>{!compact && label} <span aria-hidden="true">{side === 'right' ? '▸' : '▾'}</span></button>
