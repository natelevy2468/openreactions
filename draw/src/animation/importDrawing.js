...
export async function drawingToScene(drawing,previousScene=null,{preserveCoordinates=false}={}){
  ...
  const atoms=graph.atoms.map(a=>{
    ...
    const original=drawing.vertices?.find(v=>Math.hypot(v.x-a.x,v.y-a.y)<.01)?.animationAtomId || previousScene?.atoms.find(old=>Math.hypot(old.x-a.x,old.y-a.y)<.01)?.id;
  ...
  if(preserveCoordinates)return scene;
