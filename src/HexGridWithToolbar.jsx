...
const HexGridWithToolbar = () => {
  ...
  const drawGrid = useCallback(() => {
    ...
    vertices.forEach(v => {
      ...
      if (atom) {
        ...
        if (atom.charge) {
          ...
          ctx.fill();
          // Add black ring around the white circle
          ctx.strokeStyle = '#000000';
          ctx.lineWidth = 1;
          ctx.stroke();
    ...
    arrows.forEach((arrow, index) => {
      ...
      if (!type || type === 'arrow') {
        drawArrowOnCanvas(ctx, ox1, oy1, ox2, oy2, '#000', 3);
        ...
        drawEquilArrowOnCanvas(ctx, ox1, oy1, ox2, oy2, '#000', 3, topX1, topX2, bottomX1, bottomX2, index);
  ...
  }, [isDragging, segments, vertices, vertexAtoms, offset, mode, chargeMode, distanceToVertex, lineThreshold, vertexThreshold]);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 'max(6px, calc(min(280px, 25vw) * 0.025))', marginBottom: 0, marginTop: 'max(2px, calc(min(280px, 25vw) * 0.006))' }}>
          ...
          <button
            onClick={() => setMode(mode === 'minus' ? 'draw' : 'minus')}
            style={{
              flex: 1,
              aspectRatio: '1/1',
              display: 'flex',
          ...
          <button
            onClick={() => setMode(mode === 'lone' ? 'draw' : 'lone')}
            style={{
              flex: 1,
              aspectRatio: '1/1',
              display: 'flex',
